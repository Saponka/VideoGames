...
export const getName = (name) => {
  return async function (dispatch) {
    try {
      let response = await axios.get(`https://videogames-production-3dbe.up.railway.app/videogames/?name=${name}`);
      return dispatch({
        type: GET_NAME,
...
export const getVideoGamesId = (id) => {
  return async function (dispatch) {
    try {
      const videogames = await axios.get(`https://videogames-production-3dbe.up.railway.app/videogames/${id}`);
      return dispatch({
...
export const getByGenres = () => {
  return async function (dispatch) {
    try {
      const response = await axios.get(`https://videogames-production-3dbe.up.railway.app/genres`);
      return dispatch({
        type: "GET_BY_GENRES",
        payload: response.data,
      });
...
export const createVideogame = (videogame) => {
  return async function (dispatch) {
    try {
      const { create } = await axios.post("https://videogames-production-3dbe.up.railway.app/videogames",videogame);
      return dispatch({
